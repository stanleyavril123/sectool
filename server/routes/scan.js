...
router.post("/", async (req, res) => {
  try {
    ...
    if (!ipRegex.test(target) && !urlRegex.test(target)) {
      return res
        .status(400)
        .json({ error: "Please provide a valid URL or IP address." });
    }
    ...
    const crawlPayload = getWebServer(nmapResponse);
    console.log("Crawl Payload:", crawlPayload);
    ...
    console.error(
      `Error: ${
        error.message || "An unexpected error occurred. Please check logs"
      }`
    );
    res.status(500).json({
